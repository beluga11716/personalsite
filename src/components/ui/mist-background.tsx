 ...
 *
 * A deep zinc-toned procedural mist that responds to mouse movement.
 * Uses FBM (fractal Brownian motion) noise to create flowing, organic patterns.
 */
  ...
  useEffect(() => {
    ...
    const fsSource = /* glsl */ `
      ...
      void main() {
          ...
          vec3 mistColor = vec3(0.12, 0.16, 0.14);
          vec3 accentColor = vec3(0.18, 0.22, 0.20);

          vec3 color = mix(baseColor, mistColor, f * 0.85);
          color = mix(color, accentColor, dot(q, r) * 0.5);

          // Subtle mouse glow — faint teal tint
          float mouseGlow = smoothstep(0.40, 0.0, dist);
          color += mouseGlow * 0.07 * vec3(0.45, 0.6, 0.55);

          // Post-processing
          color = pow(color, vec3(1.08)) * 1.25;
